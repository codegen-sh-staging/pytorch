...
from torch.pattern_matcher import (
    Match,
    MULTIPLE,
    PatternMatcherPass,
    Placeholder,
    ...
    if inductor_config.pattern_matcher:
        for pass_name in inductor_config.post_grad_fusion_options:
            if pass_name == "activation_quantization_aten_pass":
                ...
                bwd_module_inputs = {node.name: node for node in bwd_module.graph.find_nodes(op="placeholder")}
                ...
                pattern_matcher_fwd_pass = AOT_QUANT_PATTERNS["activation_quantization_fwd_aten_pass"]
                ...
                trace_structured(
                    "artifact",
                    metadata_fn=lambda: {
                        "name": "activation_quantization_fwd_aten_pass",
                        "encoding": "string",
                    },
                ...
                pattern_matcher_bwd_pass = AOT_QUANT_PATTERNS["activation_quantization_bwd_aten_pass"]
                GraphTransformObserver(
                ...
                trace_structured(
                    "artifact",
                    metadata_fn=lambda: {
                        "name": "activation_quantization_bwd_aten_pass",
                        "encoding": "string",
                    },
...
def _extract_fwd_bwd_modules(
    ...
    *,
    ...
    enable_activation_quantization(saved_values, fwd_module, bwd_module, static_lifetime_input_nodes)
...
def quantize_activation_fw(match: Match, *args, **kwargs):
    ...
    for node in fwd_outputs:
        ...
        if node.meta.get("saved_for_quantization", False):
            ...
            quant_node.meta["tensor_meta"] = node.meta["tensor_meta"]._replace(dtype=quant_type)
    ...
    for node in bw_inputs:
        if is_node_meta_valid(node) and node.meta.get("saved_for_quantization", False):
            ...
            with graph.inserting_after(node):
                ...
                dequant_node.meta["val"] = node.meta["val"].detach().clone().to(dequant_type)
                dequant_node.meta["tensor_meta"] = copy.copy(node.meta["tensor_meta"]._replace(dtype=dequant_type))
            ...
            node.meta["tensor_meta"] = copy.copy(node.meta["tensor_meta"]._replace(dtype=quant_type))
