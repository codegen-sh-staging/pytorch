...
from ._inductor import config
from ._inductor.decomposition import select_decomp_table
from ._inductor.utils import is_view
...
class PatternMatcherPass:
    ...
    def apply(self, gm: Union[torch.fx.GraphModule, torch.fx.Graph]) -> int:
        ...
        if list(self.patterns.keys()) == [("placeholder", "*")]:
            input_nodes = [node for node in graph.nodes if node.op == "placeholder"]
            # use arbitrary input node to do the pattern matching
            new_target = input_nodes[0].target
            # change the patterns dict
            for key in list(self.patterns.keys()):
                target = copy.deepcopy(key[1])
                new_key = (key[0], key[1].replace(target, new_target))
                # Update the key in the dictionary
                self.patterns[new_key] = self.patterns.pop(key)
                value = self.patterns[new_key]
                for entry in value:
                    entry.pattern = Placeholder(new_target, users=MULTIPLE)
